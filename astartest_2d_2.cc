//This function shows the usage of AStar_Custom_dist(..) function to specify
// using 'taxicab' distance instead of polar distance.
...
double My_Distance(const std::vector<int> & a, const std::vector<int> & b){
    double res = 0.0;
    /*
    //Polar distance.
    for(unsigned int i=0; i<a.size(); ++i) res += static_cast<double>((b[i] - a[i])*(b[i] - a[i]));
    res = sqrt(res);
    return res;
    */
...
//    end.push_back(62); //y
    ...
    for(unsigned int i=0;i<=(BRDSIZ);++i)   for(unsigned int j=0;j<=(BRDSIZ);++j){
        thepath[i][j] = "░";
    } 

    final_path = AStar_Custom_dist(beginning, end, &My_Distance);
